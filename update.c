...
naf.point = NULL;
    ...
    && ch->position != POS_FIGHTING && ch->in_room != NULL && number_bits(1) == 0)
    {
	 ...
	 && !IS_SET(ch->act, ACT_NOTRACK) && !IS_SET(ch->nact, ACT_CLASSED))
	{
		...
		if (ch->in_room != pTrack->ch->in_room)
		ch->tracking = pTrack->ch;
	
		if ((ch->in_room == pTrack->ch->in_room) && (can_see(ch, pTrack->ch)))
		multi_hit(ch, pTrack->ch, TYPE_UNDEFINED);
	
		if ((ch->in_room == pTrack->ch->in_room) && (!can_see(ch, pTrack->ch))){
		act("$N seems to be searching for something.", ch, NULL, ch, TO_ROOM);
		ch->tracking = pTrack->ch;
		return;
		}
		}
