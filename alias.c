	...
	for (td = descriptor_list; td; td = td->next)
	    if (td->character && td->character->shunned_demon
	     && (td->character->shunned_demon == d->character))
	        write_to_descriptor(td->descriptor, "\n\r", 0);

    if (IS_NPC(ch) || ch->pcdata->alias[0] == NULL || ch->shunned_demon
    ||	!str_prefix("alias",argument) || !str_prefix("una",argument) 
    ||  !str_prefix("prefix",argument)) 
    {
        if (ch->shunned_demon)
        {
	    if (ch->shunned_demon->wait > 0)
	    {
		write_to_descriptor(d->descriptor, "The demon ignores your command.\n\r", 0);
	        return;
	    }

/*
	    for (td = descriptor_list; td; td = td->next)
		if ((td != d) && td->character && td->character->shunned_demon
		 && (td->character->shunned_demon == ch->shunned_demon))
		    write_to_descriptor(td->descriptor, "\n\r", 0);
*/
	    interpret(ch->shunned_demon, argument);
        }
	else
