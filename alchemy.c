...
void do_dye( CHAR_DATA * ch, char * argument)
{
	...
	if (get_skill(ch,gsn_brew) <= 0) {
		if (get_skill(ch,gsn_make) <= 0) {
			if (get_skill(ch,gsn_mix) <= 0) {
...
void do_discernmagic( CHAR_DATA * ch, char * argument)
{
	...
	if (power == 0)
		send_to_char("It has no alchemical power.\n\r", ch);
	if (power == 1 || power == 2 || power == 3)
		send_to_char("It has low alchemical power.\n\r", ch);
	if (power == 4 || power == 5)
		send_to_char("It has some alchemical power.\n\r", ch);
	if (power == 6 || power == 7) 	
	send_to_char("It has strong alchemical power.\n\r", ch);
	if (power == 8 || power == 9)
	send_to_char("It has very strong alchemical power.\n\r", ch);	
	if (power >= 10)
		send_to_char("It has enormous alchemical power.\n\r", ch);
	send_to_char("\n\r", ch);
