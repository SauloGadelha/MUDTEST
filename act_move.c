...
af.point = NULL;
	...
	if (chance < number_percent())
    {
	act("You fail to conceal yourself and your group behind a camouflage blind.", ch, NULL, NULL, TO_CHAR);
	WAIT_STATE(ch, UMAX(ch->wait, skill_table[gsn_camoublind].beats));
    check_improve(ch,NULL,gsn_camoublind,FALSE,1);
	return;
    }
	...
	check_improve(ch,NULL,gsn_camoublind,TRUE,1);
	return;
    }
    else
    {
