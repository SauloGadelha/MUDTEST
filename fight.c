...
void    spectralbashmob	args( ( CHAR_DATA *ch ) );
void    shieldbashmob	args( ( CHAR_DATA *ch ) );
...
void mob_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
	...
	if (is_affected(ch, gsn_spectralbashmob)){
	spectralbashmob(ch);
	return;	
	}
	
	if (is_affected(ch, gsn_shieldbashmob)){
	shieldbashmob(ch);
	return;	
	}
...
void uberskillgetsuberer(CHAR_DATA *ch, CHAR_DATA *victim,int htype,OBJ_DATA *weapon)
{
    ...
    {
        if (!IS_NPC(ch) && (weapon && weapon->value[0] == WEAPON_SWORD
	 && ch->pcdata->learned[gsn_sword] == 100)) 
        ...
        if (!IS_NPC(ch) && (weapon && obj_is_affected(weapon, gsn_devotion)))
        {
...
void raw_kill( CHAR_DATA *victim )
{
   ...
   level = victim->level;
    ...
    if ((IS_NPC(victim) || victim->pcdata->age_group < AGE_DEAD) && !is_affected(victim, gsn_phoenixdirge) && (!IS_NPC(victim) || !is_affected(victim, gsn_bilocation)))
    {
    ...
    if ((check_af = get_affect(victim, gsn_phoenixfire)) != NULL && 
		...
		(!IS_NPC(victim) || !is_affected(victim, gsn_bilocation)))
    {
...
void do_shieldbash( CHAR_DATA *ch, char *argument )
{
	...
	else if (is_affected(victim, gsn_flameshield) || is_affected(victim, gsn_lambentaura))
	{
		...
		if (IS_NPC(victim) && !IS_AFFECTED(victim, gsn_shieldbashmob)) {
		
		AFFECT_DATA af;
		af.valid = TRUE;
		af.point = NULL;
		af.where	= TO_AFFECTS;
		af.type		= gsn_shieldbashmob;
		af.level	= ch->level;
		af.duration	= 0;
		af.modifier	= 0;
		af.location	= 0;
...
if (IS_NPC(victim) && !IS_SET(victim->act, ACT_SENTINEL) || !IS_SET(victim->act, ACT_NOTRACK)){
			victim->tracking = ch;
			mob_class_aggr(victim, ch);
}
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	act("The ring of fire dissipates as $n fails to pass through it.",ch,NULL,NULL,TO_ROOM);
	act("The ring of fire dissipates as you fail to pass through it.",ch,NULL,NULL,TO_CHAR);
    ...
    //	stop_fighting( ch, TRUE );
        ...
        if (!IS_NPC(ch)) {
			...
			switch (door) {
	    case 0:
		send_to_char("You flee north.\n\r", ch); 
		break;
	    case 1:
		send_to_char("You flee east.\n\r", ch); 
		break;
	    case 2:
		send_to_char("You flee south.\n\r", ch); 
		break;
	    case 3:
		send_to_char("You flee west.\n\r", ch); 
		break;
	    case 4:
		send_to_char("You flee up.\n\r", ch); 
		break;
	    case 5:
		send_to_char("You flee down.\n\r", ch); 
		break;
		}
		}
...
void spectralbashmob(CHAR_DATA *ch){
	...
	if (!ch->fighting) {
	affect_strip(ch, gsn_spectralbashmob);
	return;
	}
...
void shieldbashmob(CHAR_DATA *ch){
	...
	if (!ch->fighting) {
	affect_strip(ch, gsn_shieldbashmob);
	return;
	}
