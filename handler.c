...
//        obj = get_eq_char(ch,loc);
	...
	case APPLY_STR:			
		set_mod_stat(ch, STAT_STR, ch->mod_stat[STAT_STR] + mod); 
		if ((ch->mod_stat[STAT_STR] + mod) > ch->max_stat[STAT_STR]) 
		set_max_stat(ch, STAT_STR, ch->max_stat[STAT_STR] + mod);
		break;
    
	case APPLY_DEX:	
		set_mod_stat(ch, STAT_DEX, ch->mod_stat[STAT_DEX] + mod); 
		if ((ch->mod_stat[STAT_DEX] + mod) > ch->max_stat[STAT_DEX]) 
		set_max_stat(ch, STAT_DEX, ch->max_stat[STAT_DEX] + mod);
		break;
	
    case APPLY_INT:	
		set_mod_stat(ch, STAT_INT, ch->mod_stat[STAT_INT] + mod); 
		if ((ch->mod_stat[STAT_INT] + mod) > ch->max_stat[STAT_INT]) 
		set_max_stat(ch, STAT_INT, ch->max_stat[STAT_INT] + mod);
		break;

    case APPLY_WIS:	
		set_mod_stat(ch, STAT_WIS, ch->mod_stat[STAT_WIS] + mod); 
		if ((ch->mod_stat[STAT_WIS] + mod) > ch->max_stat[STAT_WIS]) 
		set_max_stat(ch, STAT_WIS, ch->max_stat[STAT_WIS] + mod);
		break;

    case APPLY_CON:	
		set_mod_stat(ch, STAT_CON, ch->mod_stat[STAT_CON] + mod); 
		if ((ch->mod_stat[STAT_CON] + mod) > ch->max_stat[STAT_CON]) 
		set_max_stat(ch, STAT_CON, ch->max_stat[STAT_CON] + mod);
		break;

    case APPLY_CHR:	
		set_mod_stat(ch, STAT_CHR, ch->mod_stat[STAT_CHR] + mod); 
		if ((ch->mod_stat[STAT_CHR] + mod) > ch->max_stat[STAT_CHR]) 
		set_max_stat(ch, STAT_CHR, ch->max_stat[STAT_CHR] + mod);
		break;
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd, bool fromObj)
{
	...
	if (!check_fortitude(ch,paf,fromObj,fAdd)) { 
		set_mod_stat(ch, STAT_STR, ch->mod_stat[STAT_STR] + mod); 
		if ((ch->mod_stat[STAT_STR] + mod) > ch->max_stat[STAT_STR]) 
		set_max_stat(ch, STAT_STR, ch->max_stat[STAT_STR] + mod);
	}	
