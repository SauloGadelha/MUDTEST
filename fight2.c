 ...
 */ 
	...
	check_improve(ch,victim,gsn_circle,TRUE,1);
...
void do_ready(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_ready)) == 0)
    {
	send_to_char("Huh?\n\r", ch);
	return;
    }
	...
	/* added fail chance and improve chance here */
	
	if (chance < number_percent())
    {
	act("You fail to concentrate properly.", ch, NULL, NULL, TO_CHAR);
    check_improve(ch,NULL,gsn_ready,FALSE,1);
	return;
    }
	...
	check_improve(ch,NULL,gsn_ready,TRUE,1);
}
